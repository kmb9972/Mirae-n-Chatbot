...
.cat-html-btn:hover {{ opacity: 0.82; }}
</style>
<div class="cat-html-grid">
""")
for _name in cat_names:
    _key = CATEGORIES[_name]["key"]
    _btn_parts.append(f'<button class="cat-html-btn" onclick="window.parent.document.querySelectorAll(\'[data-testid=\\\"stButton\\\"] button\')[{list(CATEGORIES.keys()).index(_name)}].click()">{_name}</button>')
_btn_parts.append("</div>")
st.markdown("".join(_btn_parts), unsafe_allow_html=True)

# 숨겨진 Streamlit 버튼 (클릭 감지용)
st.markdown('<style>[data-testid="stHorizontalBlock"]{display:none!important}</style>', unsafe_allow_html=True)
cols = st.columns(3)
